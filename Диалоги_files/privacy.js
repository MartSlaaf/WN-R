      ...
      if (el.nextSibling) {
        if (v == Privacy.listsType) {
          ...
          for (var i in p[2]) {
            ...
            if (lists[cat_id]) str.push(cat_id < 100 ? '<a href="/friends?section=list' + cat_id + '" class="group' + color + '">' + lists[cat_id] + '</a>' : '<span class="group' + color + '">' + lists[cat_id] + '</span>');
          }
...
<table cellspacing="0" cellpadding="0">\
  ...
  <tr class="bottom_sh">\
    ...
    if (types[Privacy.listsType] && hasLists) {
      ...
      for (var i in lists) {
        var sel = hideLists ? '' : (inArray(-i, p[2]) ? '_sel' : '');
        html.push('<div class="l_item' + sel + '" id="privacy_l_item' + i + '" onclick="Privacy.choose(event, ' + Privacy.listsType + ', ' + i + ')"><div class="privacy_item_icon"></div>' + lists[i] + '</div>');
      }
      html.push('</div>');
    }
    html.push('</div>');
