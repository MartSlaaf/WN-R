              ...
              if (!(postEl = ge('post' + post_raw))) {
                ...
                if (cnt) {
                  ...
                  if (!cur.idleManager.isIdle && btnY > st + 100 && btnY < st + ch + 100) {
                    ...
                    debugLog('sort now', btnY, st + 100, st + ch + 100, stDelta);
    ...
    switch (ev_type) {
      ...
      case 'new_reply':
        ...
        var repliesEl = ge('replies' + post_id),
            ...
            newEl = se(wall.getNewReplyHTML(ev, adminLevel)),
            highlight = false;
...
<div class="feedback_group_photo_wrap fl_l" data-mid="' + e.getAttribute('data-mid') + '" data-sex="' + e.getAttribute('data-sex') + '">\
  ...
  ignoreItem: function (post_raw, feed_raw, hash) {
    ...
    ajax.post('/al_feed.php?misc', {act: 'a_ignore_item', post_raw: post_raw, feed_raw: feed_raw, hash: hash}, {
  ...
  postsGetRaws: function(el) {
    var f = domFC(el), c, m, p, r = /^post(-?\d+_\d+)$/, res = {};
    if (!f) return res;
    if (m = f.id.match(r)) {
      res[m[1]] = 1;
    } else {
  ...
  searchClickOutside: function (event) {
    if (val('feed_search') || !hasClass('feed_bar', 'feed_bar_extended_search') || isChecked('feed_search_extended_checkbox')) {
      return;
    }
  ...
  recomPreload: function() {
    ...
    ajax.post('/al_feed.php', {act: 'recom'}, {cache: 1});
  },
  recomMore: function(ev) {
    ...
    ajax.post('/al_feed.php', {act: 'recom'}, {cache: 1, onDone: function(rows) {
      hide(lnk.firstChild);
...
FeedSearch =  {
  ...
  initSelect: function () {
    ...
    if (!this.resultList || !this.resultListShadow) {
      debugLog('no containers', this.resultList, this.resultListShadow);
      return;
    }
