...
InlineDropdown.prototype._hideSubmenu = function(immediate) {
  clearTimeout(this._submenuHideTimeout);
  removeClass(this._currSubmenuItem, 'idd_hover_sublist_parent');
  var _this = this;
  function hide() {
    re(_this._els.currCascade);
    delete _this._currSubmenuItem;
  }
  if (immediate) {
    hide();
  } else {
    this._submenuHideTimeout = setTimeout(hide, 300);
  }
}
...
InlineDropdown.prototype._hoverItem = function(itemEl, andScroll, inSubmenu) {
  ...
  if (inSubmenu) {
    clearTimeout(this._submenuHideTimeout);
    addClass(_this._currSubmenuItem, 'idd_hover_sublist_parent');
  }

  // remove submenus if they are exists
  if (!inSubmenu && this._currSubmenuItem) {
  ...
  if (this._opts.sublists && this._opts.sublists[sublistId]) {
    ...
    function buildItem(item, addClass) {
      ...
      var itemHTML =    '<div class="idd_item ' + addClass + ' ' + iconCls + '" id="idd_item_' + item[0] + '" data-id="' + item[0] + '">' +
                          (sublist.withIcon && !noIcon ? ('<div class="idd_item_icon" id="' + item[0] + '"></div>') : '') +
                          checkHTML +
                          '<div class="idd_item_name">' + item[1] + '</div>' +
                        '</div>';
      return itemHTML;
    ...
    addEvent(this._els.currCascade, 'click', function(event) {
      ...
      if (item) {
        if (sublist.onSelect ? sublist.onSelect(item.getAttribute('data-id')) : true) {
          _this._hide();
        }
      }
...
ElementTooltip.prototype._onMouseLeave = function(event) {
  this._clearTimeouts();
  this._hto = setTimeout(this._hide.bind(this), this._opts.delay);
}
