...
<div class="pad_nws_btns clear_fix">\
  ...
  audioRow: function(audio, opts) {
    ...
    return rs(Pads.audioTpl, {
      ...
      playtime: audio[3],
      duration: audio[4],
      attr: 'href="/search?c[q]='+performer.replace(/(<span>|<\/span>)/g, '')+'&c[section]=audio&c[performer]=1" onclick="if (checkEvent(event)) return; Audio.selectPerformer({from_pad: true, event: event, name: \''+performer.replace(/(<span>|<\/span>)/g, '').replace(/&#39;/g, "\\\\\\&#39;")+'\'}); return false"',
      actions: actions,
      author: '',
