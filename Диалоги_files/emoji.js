...
getEmojiHTML: function(code, symbol, enabled, is_tab) {
  ...
  if (Emoji.cssEmoji[code] != undefined) {
    ...
    if (!Emoji.imgEmoji[code] && symbol && !enabled) {
      ...
      return '<img class="emoji emoji_need_load" '+(symbol ? 'alt="'+symbol+'"' : '')+' src="/images/blank.gif" data-src="/images/emoji'+(window.devicePixelRatio >= 2 ? '_2x' : '')+'/'+code+'.png" />';
    } else {
      return '<img class="emoji" '+(symbol ? 'alt="'+symbol+'"' : '')+' src="/images/emoji'+(window.devicePixelRatio >= 2 ? '_2x' : '')+'/'+code+'.png" />';
    }
  }
},
...
ttClick: function(optId, obj, needHide, needShow, ev) {
  ...
  if (!opts.isChat && getXY(obj, true)[1] < 300) {
    opts.isShortTT = 1;
  }
  var tt = opts.tt;
  if (!tt) {
    ...
    if (!opts.noStickers && window.emojiStickers !== false && window.emojiStickers !== undefined) {
      /*if (Emoji.stickers[-1] && Emoji.stickers[-1].stickers.length) {
        tabs += Emoji.getTabsCode([[-1, 1]], optId);
      }*/
  ...
  var topShift = (opts.topShift || 291) - (opts.isShortTT ? 54 : 0);
  if (Emoji.shown) {
    ...
    //hack to fix emoji window at inline-chat
    var arrow;
    if (tt.parentNode && getXY && getStyle && setStyle && geByClass && (arrow = geByClass("emoji_rpointer", tt))) {
      arrow = arrow[0];
      var width = parseInt(getStyle(tt, "width"));
      var arrowLeft = 266;
      var arrowCenter = 7;
      var needOffset = 10;
      var needX = getXY(obj)[0];
      var objWidth = parseInt(getStyle(obj, "width"));
      var parentLeft = getXY(tt.parentNode)[0];

      if (needX + objWidth / 2 < arrowLeft + arrowCenter + needOffset) {
        toParams['left'] = -width - parentLeft;
        setStyle(arrow, 'left', (needX + objWidth / 2 - needOffset) + "px");
      } else {
        toParams['left'] = '';
        setStyle(arrow, 'left', '');
      }
    ...
    if (Emoji.cssAnimation()) {
      addClass(tt, 'emoji_animation');
      setTimeout(setStyle.pbind(tt, toParams), 10);
    } else {
      setTimeout(function() {
        show(tt);
...
previewSticker: function(packId, obj, opts, ev) {
  ...
  if (obj) {
    cur.stickersDepEl = geByClass1('flat_button', obj);
  } else {
...
buyStickers: function(packId, ev, obj, hash, fromBtn) {
  ...
  ajax.post('/al_im.php', {act: 'a_stickers_buy', pack_id: packId, hash: hash, peer: peer}, {
    onDone: function(text, newStickers, btnText) {
      ...
      obj.onmouseover = '';
      addClass(obj, 'im_stickers_act_done');
      if (cur.stickersDepEl) {
        cur.stickersDepEl.innerHTML = btnText;
        cur.stickersDepEl.onmouseover = '';
        addClass(cur.stickersDepEl, 'im_stickers_act_done');
      }
